...
function App() {
  ...
  useEffect(() => {
    ...
    if (!selectedReportId || !overview.reports.some((report) => report.id === selectedReportId)) {
      setSelectedReportId(overview.reports[0].id);
    }
  }, [overview, selectedReportId]);
  ...
  const metrics = overview
    ? [
        ...
        {label: "Open incidents", value: overview.metrics.open_incidents},
        {label: "Open complaints", value: overview.metrics.open_complaints},
        {label: "Scheduled shifts", value: overview.metrics.scheduled_shifts},
      ]
    : [];
  ...
  return (
    <main className="portal-shell">
      <header className="portal-header">
        <div>
          <p className="eyebrow">Client Service View</p>
          <h1>{overview?.client.name ?? "Client Portal"}</h1>
          <p className="subtle">Attendance, patrol proof, incidents, inspections, reports, and complaints.</p>
        </div>
        <div className="header-actions">
          <button type="button" className="secondary-button" onClick={() => refreshOverview()} disabled={isLoading}>
        ...
        <section className="panel panel-wide">
          <div className="panel-header">
            <div>
              ...
              <h2>Attendance and deployments</h2>
            </div>
          </div>
