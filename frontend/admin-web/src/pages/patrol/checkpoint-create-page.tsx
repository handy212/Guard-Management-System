...
export function CheckpointCreatePage({context}: {context: PageContext}) {
  const [form, setForm] = useState({site: "", name: "", code: ""});
  return (
    <FormPage title="Create checkpoint" eyebrow="Patrol" onCancel={() => context.navigate("patrol/checkpoints")} form={
      <form className="form-grid" onSubmit={(event) => {
        event.preventDefault();
        void context.runAction(() => createCheckpoint({...form, site: Number(form.site)}, context.token), "Checkpoint created.", () => {
          setForm({site: "", name: "", code: ""});
          context.navigate("patrol/checkpoints");
        });
      }}>
        ...
        <FormActions submitting={context.isSubmitting} submitLabel="Create Checkpoint" />
      </form>
    } />
  );
}
